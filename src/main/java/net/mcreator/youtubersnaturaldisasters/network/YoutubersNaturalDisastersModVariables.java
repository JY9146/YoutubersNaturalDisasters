package net.mcreator.youtubersnaturaldisasters.network;
...
public class YoutubersNaturalDisastersModVariables {
	...
	public static void init(FMLCommonSetupEvent event) {
		...
		YoutubersNaturalDisastersMod.addNetworkMessage(PlayerVariablesSyncMessage.class, PlayerVariablesSyncMessage::buffer, PlayerVariablesSyncMessage::new, PlayerVariablesSyncMessage::handler);
	}
	...
	public static class EventBusVariableHandlers {
		...
		public static void onPlayerLoggedIn(PlayerEvent.PlayerLoggedInEvent event) {
			if (!event.getEntity().level().isClientSide()) {
				...
				if (mapdata != null)
					YoutubersNaturalDisastersMod.PACKET_HANDLER.send(PacketDistributor.PLAYER.with(() -> (ServerPlayer) event.getEntity()), new SavedDataSyncMessage(0, mapdata));
				if (worlddata != null)
					YoutubersNaturalDisastersMod.PACKET_HANDLER.send(PacketDistributor.PLAYER.with(() -> (ServerPlayer) event.getEntity()), new SavedDataSyncMessage(1, worlddata));
			}
		}
	...
	public static class WorldVariables extends SavedData {
		...
		public void syncData(LevelAccessor world) {
			...
			if (world instanceof Level level && !level.isClientSide())
				YoutubersNaturalDisastersMod.PACKET_HANDLER.send(PacketDistributor.DIMENSION.with(level::dimension), new SavedDataSyncMessage(1, this));
		}
	...
	public static class MapVariables extends SavedData {
		...
		public void syncData(LevelAccessor world) {
			...
			if (world instanceof Level && !world.isClientSide())
				YoutubersNaturalDisastersMod.PACKET_HANDLER.send(PacketDistributor.ALL.noArg(), new SavedDataSyncMessage(0, this));
		}
	...
	private static class PlayerVariablesProvider implements ICapabilitySerializable<Tag> {
		...
		public static void onAttachCapabilities(AttachCapabilitiesEvent<Entity> event) {
			if (event.getObject() instanceof Player && !(event.getObject() instanceof FakePlayer))
				event.addCapability(new ResourceLocation("youtubers_natural_disasters", "player_variables"), new PlayerVariablesProvider());
		}
	...
	public static class PlayerVariables {
		...
		public void syncPlayerVariables(Entity entity) {
			if (entity instanceof ServerPlayer serverPlayer)
				YoutubersNaturalDisastersMod.PACKET_HANDLER.send(PacketDistributor.DIMENSION.with(entity.level()::dimension), new PlayerVariablesSyncMessage(this, entity.getId()));
		}
