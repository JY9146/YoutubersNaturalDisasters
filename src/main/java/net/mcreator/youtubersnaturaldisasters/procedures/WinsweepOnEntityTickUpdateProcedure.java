...
public class WinsweepOnEntityTickUpdateProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity) {
		...
		if (!world.getEntitiesOfClass(ItemEntity.class, AABB.ofSize(new Vec3(x, y, z), 10, 10, 10), e -> true).isEmpty()) {
			...
			if (Item_item.getItem() == Blocks.DIAMOND_BLOCK.asItem() || Item_item.getItem() == Blocks.AMETHYST_BLOCK.asItem()) {
				if ((entity instanceof LivingEntity _livEnt ? _livEnt.getMainHandItem() : ItemStack.EMPTY).getCount() == 1) {
					...
					YoutubersNaturalDisastersMod.queueServerWork((int) Look_Timmer, () -> {
						...
						if (world instanceof ServerLevel _level) {
							ItemEntity entityToSpawn = new ItemEntity(_level, x, y, z, new ItemStack(YoutubersNaturalDisastersModItems.WIN_COOKIE.get()));
